    ...
    if ($Connection) {
        @(            
            @{
                ...
                value = 'IBM DB2 ODBC DRIVER - C_NIMSET~1_CLIDRI~1'
            }
            @{
                name = 'host_name'
                type = 'textbox'
...
function Open-DB2Connection {
    ...
    $connection_string =  "Driver={$($connection_params.driver_name)};Database=$($connection_params.database);Hostname=$($connection_params.host_name);Port=$($connection_params.port);Protocol=TCPIP;Uid=$($connection_params.user);Pwd=$($connection_params.password);"
